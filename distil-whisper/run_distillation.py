...
from pt_augs import do_time_stretch, do_time_masking, do_freq_masking
...
class DataTrainingArguments:
    ...
    cer_threshold: float = field(
        default=None,
        metadata={
            "help": "Filter training data with Whisper transcriptions that have greater than `cer_threshold` "
            "cer with the normalised transcriptions. This only takes effect if training on pseudo-labels targets."
            "If `--use_pseudo_labels=False`, then no cer filtering is performed, since we train directly on the text"
            "transcriptions."
        },
    )
...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], np.ndarray]]]) -> Dict[str, np.ndarray]:
        ...
        input_features = {model_input_name: [process_function(feature[model_input_name]) for feature in features]} 
...
def main():
    ...
    if training_args.do_eval:
        ...
        raw_datasets["eval"] = load_from_disk("/exp/whisper_yue/whisper_data/cv_test")
    ...
    cer_threshold = data_args.cer_threshold
    use_pseudo_labels = data_args.use_pseudo_labels
    # train_text_column_name = "whisper_transcript" if use_pseudo_labels else "text"

    # 10.3: filter training data based on cer threshold -> this is KEY to good distillation performance
    def is_cer_in_range(batch):
        ground_truth_ids = batch["labels"]
        whisper_transcript = batch["whisper_transcript"]
        ...
        if len(norm_ground_truth) > 0 and whisper_transcript is not None:
            ...
            try:
                cer = 100 * metric.compute(predictions=[norm_whisper_transcript], references=[norm_ground_truth])
                if cer < cer_threshold:
                    batch["labels"] = whisper_transcript
            except ValueError:
                return batch
        else:
            # filter automatically since we can't know the cer
            return batch
   

    # if cer_threshold is not None and use_pseudo_labels:
    #     print(len(raw_datasets["train"]))
    #     raw_datasets["train"] = raw_datasets["train"].map(is_cer_in_range)
    #     print(len(raw_datasets["train"]))
    ...
    vectorized_datasets["train"] = raw_datasets["train"]
    vectorized_datasets["eval"] = raw_datasets["eval"]
    ...
    def compute_metrics(preds, labels):
        ...
        # normalize everything and re-compute the cer
        ...
        cer = 100 * metric.compute(predictions=norm_pred_str, references=norm_label_str)
        return {"cer": cer, "cer_ortho": cer_ortho}, pred_str, label_str, norm_pred_str, norm_label_str
            ...
            if accelerator.sync_gradients:
                ...
                if training_args.do_eval and (cur_step % eval_steps == 0 or cur_step == total_train_steps):
                    ...
                    for eval_split in all_eval_splits:
                        ...
                        cer_desc = ""
                        if training_args.predict_with_generate:
                            cer_metric, pred_str, label_str, norm_pred_str, norm_label_str = compute_metrics(
                                eval_preds, eval_labels
                            )
                            eval_metrics.update(cer_metric)
                            cer_desc = " ".join([f"Eval {key}: {value} |" for key, value in cer_metric.items()])
                            log_pred(
                                accelerator,
                                pred_str,
                                label_str,
