...
def main():
    ...
    def prepare_dataset(batch):
        ...
        return batch
    raw_datasets = raw_datasets.map(prepare_dataset, num_proc=num_workers)
    ...
    # vectorized_datasets.rename_column("input_features", model_input_name)
    ...
    def compute_metrics(preds, labels, file_ids):
        ...
        return {"cer": cer, "cer_ortho": cer_ortho}, pred_str, label_str, norm_pred_str, norm_label_str, file_ids
    ...
    def eval_step_with_save():
        ...
        output_csv = os.path.join(output_dir, f"transcription.csv")
        ...
        for step, batch in enumerate(batches):
            ...
            if step % training_args.logging_steps == 0 and step > 0:
                batches.write(f"Saving transcriptions for step {step}")
        ...
        # compute cer metric for eval sets
        cer_desc = ""
        eval_preds = tokenizer.batch_decode(
            eval_preds, skip_special_tokens=True, decode_with_timestamps=return_timestamps
        )

        batches.write(f"Saving final transcriptions")
        csv_data = [[eval_ids[i], eval_preds[i]] for i in range(len(eval_preds))]
        with open(output_csv, "w", encoding="UTF8", newline="") as f:
            writer = csv.writer(f)
            # write multiple rows
        ...
        # Print metrics
        logger.info(cer_desc)

        raw_datasets = raw_datasets.add_column("whisper_transcript", eval_preds)
